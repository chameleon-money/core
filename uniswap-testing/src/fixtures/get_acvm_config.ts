...
> {
  try {
    ...
    const acvmBinaryPath = ACVM_BINARY_PATH
      ? ACVM_BINARY_PATH
      : `../../noir/${NOIR_RELEASE_DIR}/acvm`;
    ...
    logger.verbose(
      `Using native ACVM binary at ${acvmBinaryPath} with working directory ${acvmWorkingDirectory}`
    );
    ...
    const cleanup = async () => {
      if (directoryToCleanup) {
        try {
          ...
          logger.warn(
            `Failed to delete ACVM temp directory at ${directoryToCleanup}: ${err}`
          );
        }
      }
    };
