...
import { type Client, type Hex, getContract } from "viem";
...
export async function deployAndInitializeTokenAndBridgeContracts(
  ...
  if (
    !(await bridge.methods.get_config().simulate()).token.equals(token.address)
  ) {
...
export class CrossChainTestHarness {
  ...
  async mintTokensPublicOnL2(amount: bigint) {
    ...
    await this.l2Token.methods
      .mint_to_public(this.ownerAddress, amount)
      .send()
      .wait();
  }
  ...
  async consumeMessageOnAztecAndMintPrivately(
    claim: Pick<
      ...
      "claimAmount" | "claimSecret" | "messageLeafIndex" | "recipient"
    >
  ) {
