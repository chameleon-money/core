...
import type { LendingPortal } from "../typechain-types/contracts/LendingPortal";
// import { getInitialTestAccountsWallets } from "@aztec/accounts/testing";

const setup = async () => {
  const aztecModule = await import("@aztec/aztec.js");
  const createPXEClient = aztecModule.createPXEClient;
  const waitForPXE = aztecModule.waitForPXE;

  const tokenBridgeModule = await import("noir-artifacts/TokenBridge");
  const TokenBridgeContract = tokenBridgeModule.TokenBridgeContract;
  ...
  return { pxe, TokenBridgeContract };
};

describe("LendingPortal", () => {
  let lendingProtocol: LendingProtocol;
  let lendingPortal: LendingPortal;
  ...
  beforeEach(async () => {
    // Setup the PXE
    const { pxe, TokenBridgeContract } = await setup();
    // const { owner, user1, user2 } = getInitialTestAccountsWallets(pxe);
    ...
    const TokenPortalFactory = await ethers.getContractFactory("TokenPortal");
    const tokenPortal1 = await TokenPortalFactory.deploy();
    const tokenPortal2 = await TokenPortalFactory.deploy();

    // deploy the token bridge contracts
    const tokenBridge1 = TokenBridgeContract.deploy();
