...
import {Slot, SlotLib} from "../../core/libraries/TimeLib.sol";
...
abstract contract EmpireBase is IGovernanceProposer {
    ...
    function vote(
        IPayload _proposal
    ) external override(IGovernanceProposer) returns (bool) {
    ...
    function yeaCount(
        ...
        uint256 _round,
        IPayload _proposal
    ) external view override(IGovernanceProposer) returns (uint256) {
    ...
    function computeRound(
        Slot _slot
    ) public view override(IGovernanceProposer) returns (uint256) {
