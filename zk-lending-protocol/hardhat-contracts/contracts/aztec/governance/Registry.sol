...
contract Registry is IRegistry, Ownable {
    ...
    mapping(uint256 version => DataStructures.RegistrySnapshot snapshot)
        internal snapshots;
    ...
    function isRollupRegistered(
        address _rollup
    ) external view override(IRegistry) returns (bool) {
    ...
    function upgrade(
        address _rollup
    ) public override(IRegistry) onlyOwner returns (uint256) {
    ...
    function _upgrade(address _rollup) internal returns (uint256) {
        ...
        DataStructures.RegistrySnapshot memory newSnapshot = DataStructures
            .RegistrySnapshot(_rollup, block.number);
    ...
    function _getVersionFor(
        address _rollup
    ) internal view returns (uint256 version, bool exists) {
