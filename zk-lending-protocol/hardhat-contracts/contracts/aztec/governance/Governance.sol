...
import {Timestamp} from "../core/libraries/TimeLib.sol";
...
import {UserLib} from "./libraries/UserLib.sol";
...
contract Governance is IGovernance {
    ...
    mapping(uint256 proposalId => mapping(address user => DataStructures.Ballot))
        public ballots;
    mapping(address => DataStructures.User) internal users;
    mapping(uint256 withdrawalId => DataStructures.Withdrawal)
    ...
    function finaliseWithdraw(
        ...
        DataStructures.Withdrawal storage withdrawal = withdrawals[
            _withdrawalId
        ];
        require(
            !withdrawal.claimed,
    ...
    function vote(
        ...
        require(
            state == DataStructures.ProposalState.Active,
        ...
        DataStructures.Ballot storage userBallot = ballots[_proposalId][
            msg.sender
        ];
        ...
        DataStructures.Ballot storage summedBallot = proposals[_proposalId]
            .summedBallot;
        if (_support) {
    ...
    function execute(
        ...
        DataStructures.ProposalState state = getProposalState(_proposalId);
        require(
            state == DataStructures.ProposalState.Executable,
    ...
    function dropProposal(
        ...
        require(
            getProposalState(_proposalId) ==
                DataStructures.ProposalState.Dropped,
    ...
    function getProposalState(
        uint256 _proposalId
    ) public view override(IGovernance) returns (DataStructures.ProposalState) {
        require(
