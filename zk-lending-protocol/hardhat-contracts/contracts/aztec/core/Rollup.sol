...
import {IRollup, ChainTips, PublicInputArgs, L1FeeData, ManaBaseFeeComponents, FeeAssetPerEthE9, EpochRewards, BlockLog, BlockHeaderValidationFlags, FeeHeader, RollupConfigInput} from "./interfaces/IRollup.sol";
import {IStaking, ValidatorInfo, Exit, OperatorInfo, EnumerableSet} from "./interfaces/IStaking.sol";
import {IValidatorSelection} from "./interfaces/IValidatorSelection.sol";
import {FeeLib, FeeAssetValue, PriceLib} from "./libraries/rollup/FeeLib.sol";
import {HeaderLib} from "./libraries/rollup/HeaderLib.sol";
...
import {RollupCore, GenesisState, IRewardDistributor, IFeeJuicePortal, IERC20, StakingLib, TimeLib, Slot, Epoch, Timestamp, Errors, Signature, ExtRollupLib, EthValue, STFLib, RollupStore, IInbox, IOutbox} from "./RollupCore.sol";
...
contract Rollup is IStaking, IValidatorSelection, IRollup, RollupCore {
    ...
    {
        return
            ValidatorSelectionLib.getCommitteeAt(
    ...
    function getProposerForAttester(
        address _attester
    ) external view override(IStaking) returns (address) {
    ...
    function getProposerAtIndex(
        uint256 _index
    ) external view override(IStaking) returns (address) {
        return
            StakingLib
                .getStorage()
                .info[StakingLib.getStorage().attesters.at(_index)]
                .proposer;
    }

    function getInfo(
        address _attester
    ) external view override(IStaking) returns (ValidatorInfo memory) {
    ...
    function getOperatorAtIndex(
        uint256 _index
    ) external view override(IStaking) returns (OperatorInfo memory) {
        address attester = StakingLib.getStorage().attesters.at(_index);
        return
            OperatorInfo({
                proposer: StakingLib.getStorage().info[attester].proposer,
                attester: attester
            });
    }
    ...
    function getEpochCommittee(
        Epoch _epoch
    ) external view override(IValidatorSelection) returns (address[] memory) {
    ...
    function getProofSubmissionWindow()
        external
        view
        override(IRollup)
        returns (uint256)
    ...
    function getProposerAt(
        ...
        return
            ValidatorSelectionLib.getProposerAt(
                StakingLib.getStorage(),
                slot,
                epochNumber
            );
