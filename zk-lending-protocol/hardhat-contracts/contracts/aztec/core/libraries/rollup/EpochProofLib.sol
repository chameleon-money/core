...
import {SubmitEpochRootProofArgs, PublicInputArgs, IRollupCore, EpochRewards, SubEpochRewards} from "../../interfaces/IRollup.sol";
import {RollupStore, SubmitEpochRootProofArgs} from "../../interfaces/IRollup.sol";
import {Constants} from "../ConstantsGen.sol";
import {Errors} from "../Errors.sol";
import {FeeHeader, FeeLib, FeeStore} from "./FeeLib.sol";
import {STFLib, RollupStore} from "./STFLib.sol";
...
library EpochProofLib {
    ...
    function getEpochProofPublicInputs(
...

        {
            ...
            {
                bytes32 expectedPreviousBlockHash = rollupStore
                    .blocks[_start - 1]
                    .blockHash;
                require(
                    ...
                    Errors.Rollup__InvalidPreviousBlockHash(
                        ...
                        _args.previousBlockHash
                    )
                );
            }

            {
                bytes32 expectedEndBlockHash = rollupStore
                    .blocks[_end]
                    .blockHash;
                require(
                    expectedEndBlockHash == _args.endBlockHash,
                    Errors.Rollup__InvalidBlockHash(
                        expectedEndBlockHash,
                        _args.endBlockHash
                    )
                );
            }
        ...
        bytes32[] memory publicInputs = new bytes32[](
            Constants.ROOT_ROLLUP_PUBLIC_INPUTS_LENGTH +
                Constants.AGGREGATION_OBJECT_LENGTH
        );
                ...
                for (uint256 j = 0; j < Constants.BLOBS_PER_BLOCK; j++) {
                    if (j < blobsInBlock) {
                        ...
                        publicInputs[offset++] = bytes32(
                            _blobPublicInputs[blobOffset:blobOffset += 32]
                        );
                        // y
                        (
                            publicInputs[offset++],
                        ...
                        publicInputs[offset++] = bytes32(
                            uint256(
                                uint248(
                                    bytes31(
                                        _blobPublicInputs[blobOffset:blobOffset += 31]
                                    )
                                )
                            )
                        );
                        ...
                        publicInputs[offset++] = bytes32(
                            uint256(
                                uint136(
                                    bytes17(
                                        _blobPublicInputs[blobOffset:blobOffset += 17]
                                    )
                                )
                            )
                        );
...

        {
            ...
            for (
                uint256 i = 0;
                i < Constants.AGGREGATION_OBJECT_LENGTH &&
                    i < aggregationLength;
                i++
            ) {
                bytes32 part;
                assembly {
                    part := calldataload(
                        add(_aggregationObject.offset, mul(i, 32))
                    )
                }
                publicInputs[
                    i + Constants.ROOT_ROLLUP_PUBLIC_INPUTS_LENGTH
                ] = part;
            }
        }
    ...
    function handleRewardsAndFees(
        ...
        bool isFeeCanonical = address(this) ==
            rollupStore.config.feeAssetPortal.canonicalRollup();
        ...
        if (isFeeCanonical || isRewardDistributorCanonical) {
            ...
            if (length > $er.longestProvenLength) {
...

                {
                    ...
                    uint256 blockRewardsAvailable = isRewardDistributorCanonical
                        ? rollupStore
                            .config
                            .rewardDistributor
                            .claimBlockRewards(address(this), added)
                        : 0;
                ...
                for (uint256 i = $er.longestProvenLength; i < length; i++) {
                    FeeHeader storage feeHeader = feeStore.feeHeaders[
                        _args.start + i
                    ];

                    (uint256 fee, uint256 burn) = isFeeCanonical
                        ? (
                            uint256(_args.fees[1 + i * 2]),
                            feeHeader.congestionCost * feeHeader.manaUsed
                        )
                        : (0, 0);
                    ...
                    v.proverFee = Math.min(
                        feeHeader.manaUsed * feeHeader.provingCost,
                        fee - burn
                    );
...

                    {
                        ...
                        rollupStore.sequencerRewards[v.sequencer] += (v
                ...
                if (t.totalBurn > 0) {
                    rollupStore.config.feeAsset.transfer(
                        BURN_ADDRESS,
                        t.totalBurn
                    );
                }
    ...
    function assertAcceptable(
        ...
        Slot deadline = startEpoch.toSlots() +
        ...
        require(
            ...
            Errors.Rollup__PastDeadline(
                deadline,
                Timestamp.wrap(block.timestamp).slotFromTimestamp()
            )
        );
    ...
    function checkBlobPublicInputsHashes(
        ...
        bytes32 calcBlobPublicInputsHash = sha256(
            abi.encodePacked(
                _blobPublicInputs[_index:_index +
        ...
        require(
            ...
            Errors.Rollup__InvalidBlobPublicInputsHash(
                _blobPublicInputsHash,
                calcBlobPublicInputsHash
            )
        );
    }
